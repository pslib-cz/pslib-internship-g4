...
const InternshipClassCountTable:FC<InternshipClassCountTableProps> = ({setId}) => {
    ...
    useEffect(() => {
        const fetchData = async () => {
          try {
            ...
            const response = await fetch(`/api/summaries/classrooms?${queryString}`)
        ...
        <Table>
            ...
            <Table.Tbody>
                <Table.Tr>
                    ...
                    <Table.Td>{data.reduce((acc, row) => acc + row.totalInternships, 0)}</Table.Td>
                </Table.Tr>
                <Table.Tr>
                    ...
                    <Table.Td>{data.reduce((acc, row) => acc + row.uniqueStudents, 0)}</Table.Td>
                </Table.Tr>
            </Table.Tbody>
        </Table>
    );
