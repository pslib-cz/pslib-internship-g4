...
export async function POST(request: NextRequest) {
    ...
    if (process.env.GEOCODING_ENGINE === "mapy.cz") {
        ...
        try {
            ...
            const response = await fetch(`https://api.mapy.cz/v1/geocode?query=${body.street ? body.street : ""} ${body.descNumber ? body.descNumber : ""}/${body.orientNumber? body.orientNumber : ""}, ${body.municipality ? body.municipality : ""}, ${body.postalCode ? body.postalCode : ""}&limit=1&apikey=${process.env.MAPY_CZ_KEY}`)
            ...
            if (response && response.ok) {
                ...
                const structure: Array<RegionalStructureItem> = data.items[0].regionalStructure;
                const coord: PlaceCoordinates = { 
                    lat: place.lat, 
                    lon: place.lon,
                    country: structure.find(s => s.type === "regional.country")?.name ?? null,
                    municipality: structure.find(s => s.type === "regional.municipality")?.name ?? null,
                    street: structure.find(s => s.type === "regional.street")?.name ?? null,
                    address: structure.find(s => s.type === "regional.address")?.name ?? null
                };
                return new Response(JSON.stringify(coord), { status: 200 });
            } else {
                return new Response("Error fetching data:" + response.status, { status: 500 });
            }
        } catch (error) {
