...
export async function GET(request: NextRequest, {params}: {params: {id: number, tagId: number}}) {
...
export async function DELETE(request: NextRequest, {params}: {params: {id: number, tagId: number}}) {
    const session = await auth();
    const id = Number(params.id)
    const tag = Number(params.tagId);
