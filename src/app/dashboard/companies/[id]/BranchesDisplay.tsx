...
import { Table, TableThead, TableTbody, TableTr, Text, Alert, ActionIcon, Modal, Group, Button } from "@mantine/core"
...
const BranchesDisplay = ({id, switchModeAction, setEditIdAction}:{id: number, switchModeAction: (mode: displayMode) => void, setEditIdAction: (x: number) => void}) => {
    const [branches, setBranches] = useState<CompanyBranchWithLocation[] | null>(null);
    ...
    if (branches?.length === 0) {
        return (
            <Text>Firma nemá žádné pobočky</Text>
        );
    }
    return (
        <>
                ...
                <TableTbody>
                    {branches?.map((branch, index) => (
                        <TableTr key={index}>
                            ...
                            /></Table.Td>
                            <Table.Td>
                                <ActionIcon variant="light" color="red" onClick={() => {setLocationId(branch.locationId); open();} }><IconTrash /></ActionIcon>{" "}
                                <ActionIcon variant="light" onClick={() => {setEditIdAction(branch.locationId); switchModeAction("EDIT") }}><IconEdit /></ActionIcon>
                            </Table.Td>
                        </TableTr>
                    ))}
                </TableTbody>
            ...
            >
                ...
                <Group mt="xl">
                    ...
                    }} color="red" leftSection={<IconTrash />}>Smazat</Button>
                    <Button onClick={close} variant='default'>Storno</Button>
                </Group>
            </Modal>
        </>
    );
